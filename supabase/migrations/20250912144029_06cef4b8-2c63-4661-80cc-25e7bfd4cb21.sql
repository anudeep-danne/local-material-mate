-- First, let's create a comprehensive RawMate 2.0 database schema
...
-- Create batches table for farmer batch creation
...
-- Create transfers table to track batch movement through supply chain
...
CREATE TABLE IF NOT EXISTS public.consumer_orders (
  ...
  status TEXT NOT NULL DEFAULT 'Pending' CHECK (status IN ('Pending', 'Confirmed', 'Packed', 'Out for Delivery', 'Delivered', 'Cancelled')),
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);
...
-- Create shipments table for tracking transport
...
CREATE POLICY "All authenticated users can view available batches"
ON public.batches
FOR SELECT
USING (true);
...
CREATE POLICY "Users can create transfers they are involved in"
ON public.transfers
FOR INSERT
...
CREATE POLICY "All authenticated users can view retailer inventory"
ON public.retailer_inventory
FOR SELECT
USING (true);

-- Create RLS policies for consumer orders
CREATE POLICY "Consumers can manage their own orders"
ON public.consumer_orders
FOR ALL
USING (consumer_id = auth.uid())
WITH CHECK (consumer_id = auth.uid());

CREATE POLICY "Retailers can view orders for their products"
ON public.consumer_orders
FOR SELECT
USING (retailer_id = auth.uid());

CREATE POLICY "Retailers can update orders for their products"
ON public.consumer_orders
FOR UPDATE
USING (retailer_id = auth.uid());

-- Create RLS policies for offers
CREATE POLICY "Farmers can view offers for their batches"
ON public.offers
FOR SELECT
USING (farmer_id = auth.uid());

CREATE POLICY "Farmers can update offers for their batches"
ON public.offers
FOR UPDATE
USING (farmer_id = auth.uid());

CREATE POLICY "Distributors can manage their own offers"
ON public.offers
FOR ALL
...
CREATE POLICY "Distributors can manage their own shipments"
ON public.shipments
FOR ALL
USING (distributor_id = auth.uid())
WITH CHECK (distributor_id = auth.uid());

CREATE POLICY "All users can view shipments they are involved in"
ON public.shipments
FOR SELECT
USING (
...
CREATE TRIGGER update_retailer_inventory_timestamp
BEFORE UPDATE ON public.retailer_inventory
FOR EACH ROW
EXECUTE FUNCTION public.update_timestamp();

CREATE TRIGGER update_consumer_orders_timestamp
